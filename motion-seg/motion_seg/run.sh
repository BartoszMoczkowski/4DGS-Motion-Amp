...
# extract_trajectories.py needs the GPU (full 4dgs-core env); segment_rigid.py and
# evaluate_segmentation.py are pure numpy/scipy (motion-seg base install).
#
...
CONFIG="core/arguments/multipleview/$NAME.py"
...
uv run --package motion-seg python -m motion_seg.segment_rigid \
...
if [ -f "$DATA_DIR/gt_segmentation.npz" ]; then
    ...
    uv run --package motion-seg python -m motion_seg.evaluate_segmentation \
