...
def main():
    ...
    try:
        ...
        png_path = (base or out_path) + "_preview.png"
        render_segmentation_png(canonical_xyz, labels, png_path)
        print(f"[ok] wrote {png_path}")
    except Exception as e:  # pragma: no cover -- best-effort, don't fail the run over a preview
        print(f"[warn] preview PNG failed: {e}", file=sys.stderr)
