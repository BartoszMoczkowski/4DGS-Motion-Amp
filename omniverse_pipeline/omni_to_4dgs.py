...
def qvec_to_rotmat(q: np.ndarray) -> np.ndarray:
    """Quaternion (w,x,y,z) -> 3x3 rotation. Mirror of scene/colmap_loader.qvec2rotmat."""
    w, x, y, z = q
    return np.array([
...
# ----------------------------------------------------------------------------- #
#  Self-test: synthetic cameras -> write bins -> read back -> verify poses
# ----------------------------------------------------------------------------- #

def _selftest():
    """Round-trip: build known OpenCV cameras, write COLMAP bins, re-read with the
    SAME logic scene/colmap_loader uses, reconstruct c2w, assert it matches."""
    import tempfile
    ...
    for i in range(n):
        ang = 2*np.pi*i/n
        C = np.array([3*np.cos(ang), 3*np.sin(ang), 1.0])
        fwd = -C/np.linalg.norm(C)                     # look at origin
        world_up = np.array([0, 0, 1.0])
        right = np.cross(fwd, world_up); right /= np.linalg.norm(right)
        down = np.cross(fwd, right)                     # OpenCV: +Y down
        R_c2w = np.stack([right, down, fwd], axis=1)
        c2w = np.eye(4); c2w[:3, :3] = R_c2w; c2w[:3, 3] = C
        cams.append(c2w)

    d = tempfile.mkdtemp()
    colmap = [dict(zip(("qvec", "tvec"), opencv_c2w_to_colmap_qt(c)),
                   name=f"image{i+1}.jpg") for i, c in enumerate(cams)]
    write_images_bin(os.path.join(d, "images.bin"), colmap)
    write_cameras_bin(os.path.join(d, "cameras.bin"), 1280, 720, 1000, 1000, 640, 360)

    # Re-read images.bin exactly like scene/colmap_loader.read_extrinsics_binary
    ok = True
    with open(os.path.join(d, "images.bin"), "rb") as f:
        ...
        for k in range(ni):
            ...
            nm = b""; ch = f.read(1)
            while ch != b"\x00": nm += ch; ch = f.read(1)
            n2d = struct.unpack("<Q", f.read(8))[0]
            # Reconstruct c2w the way multipleview_dataset does: R = qvec2rotmat(q).T, T = tvec
            R = qvec_to_rotmat(qvec).T                  # this is R_c2w
            C = -R @ tvec                               # camera center
            err_R = np.linalg.norm(R - cams[k][:3, :3])
            err_C = np.linalg.norm(C - cams[k][:3, 3])
            if err_R > 1e-6 or err_C > 1e-6:
                ok = False
                print(f"  cam {k}: name={nm.decode()} err_R={err_R:.2e} err_C={err_C:.2e}")
            else:
                print(f"  cam {k}: name={nm.decode()} OK (err_R={err_R:.1e}, err_C={err_C:.1e})")

    # LLFF round-trip: build row, apply loader remap, check axes
    row = opencv_c2w_to_llff_row(cams[0], 720, 1280, 1000, 0.1, 100)
    p = row[:15].reshape(3, 5)
    remap = np.concatenate([p[:, 1:2], -p[:, 0:1], p[:, 2:4]], -1)  # loader's line
    right_r, up_r = remap[:, 0], remap[:, 1]
    R0 = cams[0][:3, :3]
    err_llff = np.linalg.norm(right_r - R0[:, 0]) + np.linalg.norm(up_r - (-R0[:, 1]))
    print(f"  LLFF remap axis error: {err_llff:.2e} {'OK' if err_llff < 1e-6 else 'FAIL'}")
    ok = ok and err_llff < 1e-6

    # PLY round-trip
    xyz = rng.uniform(-1, 1, (50, 3)); rgb = rng.randint(0, 256, (50, 3), np.uint8)
    write_ply(os.path.join(d, "t.ply"), xyz, rgb/255.0)
    x2, r2 = _read_ply_xyz_rgb(os.path.join(d, "t.ply"))
    err_ply = np.abs(x2 - xyz).max()
    print(f"  PLY round-trip xyz err: {err_ply:.2e} {'OK' if err_ply < 1e-5 else 'FAIL'}")
    ok = ok and err_ply < 1e-5
...
if __name__ == "__main__":
    ...
    ap.add_argument("--capture", help="capture dir produced by omni_capture.py")
    ap.add_argument("--out", default=".", help="4DGS repo root (writes data/multipleview/<name>)")
    ap.add_argument("--name", default="omni_scene", help="scene name")
    ap.add_argument("--near", type=float, default=0.1)
    ap.add_argument("--far", type=float, default=1000.0)
    ap.add_argument("--target-radius", type=float, default=4.0,
                     help="rescale scene (after meters_per_unit conversion) so the nerf++-style "
                          "camera radius equals this (matches the scale 4DGS's hardcoded "
                          "learning rates assume). 0 = only apply meters_per_unit, no rescale.")
    ap.add_argument("--selftest", action="store_true", help="run internal round-trip tests")
    args = ap.parse_args()
    if args.selftest:
        sys.exit(_selftest())
    if not args.capture:
        ap.error("--capture is required (or use --selftest)")
    convert(args.capture, args.out, args.name, args.near, args.far, args.target_radius)
