...
test -f "core/arguments/multipleview/$NAME.py" || { echo "ERROR: core/arguments/multipleview/$NAME.py not found"; exit 1; }

echo "[train] 4DGS on data/multipleview/$NAME"
uv run --package 4dgs-core python core/train.py -s "data/multipleview/$NAME" --port "$PORT" \
    --expname "multipleview/$NAME" --configs "core/arguments/multipleview/$NAME.py"

echo "[render] test + video views"
uv run --package 4dgs-core python core/render.py --model_path "output/multipleview/$NAME/" --skip_train \
    --configs "core/arguments/multipleview/$NAME.py"

echo "[done] trained model in output/multipleview/$NAME/"
echo "  next: segmentation (MBS / trajectory clustering) then per-segment motion amp (core/render_amp.py)."
