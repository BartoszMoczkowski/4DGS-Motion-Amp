...
under ``omniverse-pipeline/omniverse_pipeline/``/``motion-seg/motion_seg/`` (see ``pipeline.vendored``'s module docstring for
why this exists instead of importing/subprocessing those scripts directly):

- :mod:`pipeline.vendored.host.convert` — ported from ``omniverse-pipeline/omniverse_pipeline/omni_to_4dgs.py``'s
...
- :mod:`pipeline.vendored.host.rigidity_graph` / :mod:`pipeline.vendored.host.segment_rigid` —
  ported from ``motion-seg/motion_seg/rigidity_graph.py`` / ``motion-seg/motion_seg/segment_rigid.py``'s
  ``segment_trajectories()``.
- :mod:`pipeline.vendored.host.metrics` / :mod:`pipeline.vendored.host.seg_eval` — ported from
  ``motion-seg/motion_seg/metrics.py`` / ``motion-seg/motion_seg/evaluate_segmentation.py``'s ``evaluate()`` /
  ``_write_colored_ply()``.
