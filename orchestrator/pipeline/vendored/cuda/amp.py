"""Vendored, verbatim copy of the repo-root ``render_amp.py`` (T09 copy-in, per
...
Known pre-existing quirk, kept as-is (copy, don't fix — same policy as T07's documented Otsu-
threshold bug in ``rigidity_graph.py``): ``--amp_factors`` is declared ``type=int`` even though
