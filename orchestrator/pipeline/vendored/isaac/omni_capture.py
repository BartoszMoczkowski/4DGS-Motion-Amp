"""Vendored, verbatim copy of ``omniverse-pipeline/omniverse_pipeline/omni_capture.py`` (T11 copy-in, per
...
inside the ``isaac`` container via Isaac Sim's own bundled interpreter (``/isaac-sim/python.sh``,
never the container's plain ``python`` — only that interpreter has ``isaacsim``/``omni.*``/``pxr``
wired up), by ``pipeline/stages/capture_isaac.py`` (see ``pipeline/stages/isaac_common.py`` for the
shared CLI-exec plumbing). Never imported by the orchestrator's own host process — cannot be
(needs a GPU + the Isaac Sim runtime, per its own module docstring below).

No changes from the reference script. Its ``import rig as rigmod`` (after
``sys.path.insert(0, dirname(__file__))``) resolves unchanged: ``rig.py`` is vendored alongside it
in this same directory (see ``pipeline.vendored.isaac.rig``'s docstring).

``pipeline/stages/capture_isaac.py`` writes the ``--config`` YAML from the resolved
``CaptureConfig`` (T02) and overrides ``scene.usd_path``/``output.capture_dir`` via this script's
own ``--usd``/``--out`` CLI flags (already built into ``_parse_args`` below) with the DAG's actual
``animated_mesh`` input artifact / this run's own capture directory — the same "derive from the
DAG's artifact wiring, not the static config value" pattern ``train.default`` uses for
``source_path``/``model_path`` (see ``pipeline.config.bridge``'s docstring).
