"""Vendored, verbatim copy of ``omniverse-pipeline/omniverse_pipeline/add_motion.py`` (T11 copy-in, per
...
inside the ``isaac`` container (``pipeline/stages/prep_motion.py`` builds the CLI invocation via
``/isaac-sim/python.sh``, see ``pipeline/stages/isaac_common.py``), never imported by the
orchestrator's own host process.

No changes from the reference script. Needs ``pxr`` (usd-core) + numpy; ``--plot``/matplotlib is
only imported if ``--plot`` is actually passed (``PrepMotionConfig.plot``, opt-in, same pattern as
``split_mesh.py``'s ``--preview``).
