...
inside the ``isaac`` container (``pipeline/stages/prep_split.py`` builds the CLI invocation via
``/isaac-sim/python.sh``, see ``pipeline/stages/isaac_common.py``), never imported by the
orchestrator's own host process.

No changes from the reference script. Needs ``pxr`` (usd-core, present via Isaac Sim's bundled
interpreter) + ``trimesh`` (not preinstalled in the ``isaac`` image — a one-time manual
``pip install trimesh`` inside the container, see ``planning/WINDOWS_SETUP.md``'s Isaac stages
setup step) + numpy; ``--preview``/matplotlib is only imported if ``--preview`` is actually passed
(``PrepSplitConfig.preview``, opt-in, mirrors ``seg_eval.default``'s ``recolored_ply`` handling).

``PrepSplitConfig.weld_tol`` (``pipeline.config.models``) has no effect: this script's
``split_components()`` takes a ``weld_tol`` parameter, but ``main()`` never passes it through (a
pre-existing gap in the reference script itself, not introduced by this port) — the config schema
anticipated a CLI flag that doesn't actually exist. Not fixed here per the copy-in rule ("copy the
logic in, don't rewrite"); see ``pipeline/stages/prep_split.py``'s note.
