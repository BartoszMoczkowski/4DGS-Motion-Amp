...
class IsaacStageError(RuntimeError):
    ...
    ``log_path`` is a real constructor attribute, not just embedded in the message string —
    mirrors ``pipeline.stages.cuda_common.CudaStageError``'s T12 addition, so
    ``pipeline.resources.oom_retry.is_oom_error`` can read the captured output directly.
    """
