...
- **Layer 3 — `orchestrator/ui/`**: single-file Streamlit app (Presets, Launch & Monitor, Runs & Artifacts, Compare Runs, GPU/Containers), importing Layer 1 in-process and reusing the MCP server's job/artifact-view modules so UI and Claude see identical shapes. Supersedes `amp-ui/amp_ui/ampUI.py`'s folder-scanning workflow.
